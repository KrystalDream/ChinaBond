...
#define kRKKLineCalLineWidth 2

@interface RKKLine ()<UIScrollViewDelegate>
...
//        [self addGestureRecognizer:pinGesture];
        ...
        double xMax =  1.5f;
        NSString *xMaxStr = [NSString stringWithFormat:@"%.1lf", xMax];
        ...
        double y1Max =  5.5f;
        NSString *y1MaxStr = [NSString stringWithFormat:@"%.1lf", y1Max];
        ...
        if (xArr && yArr && [xArr count]>0 && [yArr count]>0) {
            [self configWithXArr:xArr yArr:yArr];
        }
                   ...
                   yArr:(NSArray *)yArr
{
    ...
    self.baseRightBottomPoint = CGPointMake(self.bounds.size.width*self.chartScaleNum - 2, self.baseOriginPoint.y);
...
- (void)refreshScrollViewWhenScale
{
    ...
    self.baseXMaxValue = [[xInputDataAfterSort lastObject] floatValue];
    
    self.kLineScrollView.contentSize = CGSizeMake(self.kLineScrollView.bounds.size.width*self.chartScaleNum, self.kLineScrollView.bounds.size.height);
...
- (void)refreshWithXMinValue:(CGFloat)xMinValue xMaxValue:(CGFloat)xMaxValue
{
    ...
    if (yFirst+0.5<[[yDataAfterSort firstObject] floatValue]) {
        yFirst += 0.5;
    }
    float yLast = ceil([[yDataAfterSort lastObject] floatValue]);
    if (yLast-0.5>[[yDataAfterSort lastObject] floatValue]) {
        yLast -= 0.5;
    }
    NSMutableArray *yNeedDisplay = [NSMutableArray arrayWithCapacity:0];
    for (float i=yFirst; i<yLast+0.1; i+=0.5) {
        [yNeedDisplay addObject:[NSNumber numberWithFloat:i]];
    }
    self.yDataSource = yNeedDisplay;
    
    NSArray *xNeedDisplay  = @[@0, @0.5, @3, @5, @9, @15, @20, @30, @50];
    if (self.chartScaleNum>1) {
        xNeedDisplay = @[@0, @0.5, @3, @5, @9, @15, @20, @25, @30, @35, @40, @45, @50];
    }
    NSMutableArray *xNeedArrTmp = [NSMutableArray arrayWithCapacity:0];
    for (NSNumber *xNeedD in xNeedDisplay) {
        if ([xNeedD floatValue]<xMinValue || [xNeedD floatValue]>xMaxValue) {
            continue;
        }
        [xNeedArrTmp addObject:xNeedD];
    }
    self.xDataSource = xNeedArrTmp;
...
- (void)drawRect:(CGRect)rect
{
    ...
    for (NSInteger index=0; index < self.xDataSource.count; index++) {
        ...
        NSString *valStr = [NSString stringWithFormat:@"%.0lf", [num floatValue]];
        int numInt = (int)[num floatValue];
        if ([num floatValue]-numInt>0) {
            valStr = [NSString stringWithFormat:@"%.1lf", [num floatValue]];
        }
        ...
        if (!(xPosition < self.baseOriginPoint.x) && !(xPosition > self.baseRightBottomPoint.x)) {
            ...
            if (index==[self.xDataSource count]-1) {
                titleX = xPosition - (title1Size.width);
            }
    ...
    for (NSInteger i = 0; i < self.yDataSource.count; i++) {
        ...
        CGFloat y1Position = 0;
        
        if ((self.yMaxValue-self.yMinValue)>0) {
            y1Position = self.baseOriginPoint.y + (([num floatValue]-self.yMinValue)/(self.yMaxValue-self.yMinValue))*(self.baseLeftTopPoint.y - self.baseOriginPoint.y);
        }
...
{
    self.calVerTextContentLayer.string = [NSString stringWithFormat:@"%.1lf",xCorrectValue];
    self.calHorTextContentLayer.string = [NSString stringWithFormat:@"%.1lf",yCorrectValue];
}
#pragma mark - 竖向CAL线底端文字的绘制
- (void)addCalVerTextLayerOfXInView:(CGFloat)xInView
{
    self.calVerTextLayer = [CALayer layer];
    self.calVerTextLayer.frame = CGRectMake(xInView-23/2, self.baseOriginPoint.y+3, 23, 14);
    UIColor *color = RKFillColor;
...
- (void)addCalHorTextLayerOfYInView:(CGFloat)yInView
{
    ...
    self.calHorTextLayer.frame = CGRectMake(self.baseOriginPoint.x-3-23, yInView-14/2, 23, 14);
    UIColor *color = RKFillColor;
...
- (void)kLineLongPressed:(UILongPressGestureRecognizer *)sender
{
    ...
    if (sender.state == UIGestureRecognizerStateBegan) {
        ...
        CGFloat xValueAfterCorrect = [[self correctXValue:xValue] floatValue];
...
- (NSString *)correctXValue:(CGFloat)xValue
{
    int xTmp = (int)(xValue*10);
    float ret = xTmp/10.0;
    NSNumberFormatter *numberFormatter = [[NSNumberFormatter alloc] init];
    [numberFormatter setPositiveFormat:@"0.0"];
    return [numberFormatter stringFromNumber:[NSNumber numberWithFloat:ret]];
}
...
- (void)addCalVerLineOfXInView:(CGFloat)xInView
{
    ...
    self.calVerLayer.lineWidth = kRKKLineCalLineWidth;
...
- (void)addCalHorLineOfYInView:(CGFloat)yInView
{
    ...
    self.calHorLayer.lineWidth = kRKKLineCalLineWidth;
...
{
    CGFloat sitePercent = 0;//(yValue-self.yMinValue)/(self.yMaxValue-self.yMinValue);
    
    if ((self.yMaxValue-self.yMinValue)>0) {
        sitePercent = (yValue-self.yMinValue)/(self.yMaxValue-self.yMinValue);
    }
    else
    {
        sitePercent = 0;
