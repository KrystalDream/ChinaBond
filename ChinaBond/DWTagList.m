...
#define BORDER_COLOR UIColorFromRGB(0xdbdbdb).CGColor
...
- (void)display
{
    ...
    for (NSDictionary *text in textArray) {
        CGSize textSize = [text[@"keyWord"] sizeWithFont:[UIFont systemFontOfSize:FONT_SIZE]
                           constrainedToSize:CGSizeMake(self.frame.size.width, 1500)
        ...
        if (!lblBackgroundColor) {
            ...
            label.dk_backgroundColorPicker = DKColorWithRGB(0xffffff, 0x0f0f0f);
        } else {
            [label setBackgroundColor:lblBackgroundColor];
        }
        label.dk_textColorPicker = DKColorWithRGB(0xdbdbdb, 0x8c8c8c);
        [label setText:text[@"keyWord"]];
