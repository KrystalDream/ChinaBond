...
#import "CBUploadParam.h"
#import "MBProgressHUD.h"
#import <AFHTTPSessionManager.h>
@implementation CBNetworkManager
+ (instancetype)allocWithZone:(struct _NSZone *)zone
{
    static CBNetworkManager *_manager;
                 ...
                 showHUD:(BOOL)showHUD
{
    ...
    [[CBAFNNetworkHandler sharedInstance] conURL:url networkType:CBAFNNetWorkGET params:mutableDict delegate:delegate showHUD:showHUD target:target action:action successBlock:successBlock failureBlock:failureBlock];
}
/**
 *   GET请求通过Block 回调结果
 *
...
+ (void)getRequstWithURL:(NSString*)url
                  params:(NSDictionary*)params
                delegate:(id<CBAFNNetworkDelegate>)delegate
                 showHUD:(BOOL)showHUD
{
...
{
    NSMutableDictionary *mutableDict = [NSMutableDictionary dictionaryWithDictionary:params];
    [[CBAFNNetworkHandler sharedInstance] conURL:url networkType:CBAFNNetWorkPOST params:mutableDict delegate:delegate showHUD:showHUD target:target action:action successBlock:successBlock failureBlock:failureBlock];
}
/**
 *   通过 Block回调结果
 *
...
+ (void)uploadFileWithURL:(NSString*)url
             ...
             successBlock:(MHAsiSuccessBlock)successBlock
             failureBlock:(MHAsiFailureBlock)failureBlock
              uploadParam:(CBUploadParam *)uploadParam
                  showHUD:(BOOL)showHUD
{
    if (showHUD) {
        [MBProgressHUD showHUDAddedTo:nil animated:YES];
    }
    CBLog(@"上传图片接口 URL-> %@",url);
    CBLog(@"上传图片的参数-> %@",paramsDict);
